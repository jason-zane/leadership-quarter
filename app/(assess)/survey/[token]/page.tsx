...
export default async function InvitedSurveyPage({ params }: Props) {
  ...
  const response = await fetch(`${baseUrl}/api/assessments/invitation/${encodeURIComponent(token)}`, {
    cache: 'no-store',
  }).catch(() => null)
  ...
  if (!response?.ok || !payload?.ok) {
    return (
      <div className="mx-auto max-w-3xl px-6 pb-24 pt-8 text-[var(--site-text-primary)] md:px-12">
