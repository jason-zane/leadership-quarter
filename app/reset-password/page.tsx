...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    async function prepareSession() {
      ...
      // Fallback for token_hash recovery links.
      if (tokenHash && type) {
      ...
      if (!sessionEmail) {
        if (hasLinkCredentials && lastAuthError) {
          setError(`Reset link is invalid or expired. ${lastAuthError}`)
        } else {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setTimeout(() => {
      window.location.assign(
        `${getPublicSiteUrl()}/client-login?message=${encodeURIComponent('Password updated. Please log in.')}`
      )
    }, 1200)
  }
