...
export default function SetPasswordPage() {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!ready) {
      setError('Auth session is not ready. Open the invite link again.')
      return
    }
    ...
    if (updateError) {
      if (updateError.message.toLowerCase().includes('auth session missing')) {
        setError('Setup link is invalid or expired. Ask an admin to send a new invite.')
      } else {
    ...
    <div className="flex min-h-screen items-center justify-center bg-zinc-50 dark:bg-black">
      <div className="w-full max-w-sm rounded-lg bg-white p-8 shadow-sm dark:bg-zinc-900">
        ...
        <form onSubmit={onSubmit} className="flex flex-col gap-4">
          ...
          >
            {saving ? 'Saving...' : !ready ? 'Preparing...' : 'Set Password'}
          </button>
        </form>
      </div>
    </div>
