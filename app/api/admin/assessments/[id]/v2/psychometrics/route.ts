...
import {
  getAdminAssessmentV2PsychometricsWorkspace,
  saveAdminAssessmentV2PsychometricsConfig,
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await getAdminAssessmentV2PsychometricsWorkspace({
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await saveAdminAssessmentV2PsychometricsConfig({
    adminClient: auth.adminClient,
    assessmentId: id,
