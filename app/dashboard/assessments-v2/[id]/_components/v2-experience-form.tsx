...
export function V2ExperienceForm({ assessmentId }: Props) {
        ...
        actions={(
          <div className="flex flex-col items-end gap-2">
            <div className="flex items-center gap-3">
              {assessmentKey ? (
                <a
                  href={`/assess/p/${encodeURIComponent(assessmentKey)}?engine=v2`}
        ...
        {activeEditorTab === 'build_runtime' ? (
          <FoundationSurface className="space-y-5 p-6">
            <SectionHeader
              ...
              description="Configure how the assessment route is enabled, what completion does, and how support details appear."
            />

            <label className="flex items-start gap-3 rounded-2xl border border-[rgba(103,127,159,0.12)] bg-white/70 p-4">
              <input
                type="checkbox"
                checked={reportConfig.v2_runtime_enabled}
                onChange={(event) => setReportConfig((current) => ({ ...current, v2_runtime_enabled: event.target.checked }))}
                className="mt-1 h-4 w-4 rounded border-zinc-300"
              />
              <span>
                <span className="block text-sm font-medium text-[var(--admin-text-primary)]">Enable assessment route</span>
                <span className="block text-sm text-[var(--admin-text-muted)]">
                  Required before this workspace can open the assessment flow.
                </span>
              </span>
            </label>
