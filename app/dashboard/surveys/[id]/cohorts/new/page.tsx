...
export default function NewSurveyCohortPage() {
  ...
  async function create(send: boolean) {
    ...
    const cohortResponse = await fetch(`/api/admin/surveys/${surveyId}/cohorts`, {
      method: 'POST',
    ...
    if (invitations.length > 0) {
      await fetch(`/api/admin/surveys/${surveyId}/cohorts/${cohortBody.cohort.id}/invitations`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    router.push(`/dashboard/surveys/${surveyId}/cohorts/${cohortBody.cohort.id}`)
  }
