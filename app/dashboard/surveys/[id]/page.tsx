...
export default async function SurveyOverviewPage({ params }: Props) {
  ...
  try {
    const [sRes, rcRes, pcRes, rrRes] = await Promise.all([
      adminClient
        ...
        .eq('id', id)
        .maybeSingle(),
      adminClient.from('survey_submissions').select('id', { count: 'exact', head: true }).eq('survey_id', id),
      adminClient.from('survey_invitations').select('id', { count: 'exact', head: true }).eq('survey_id', id).eq('status', 'pending'),
      adminClient
    ...
    survey = sRes.data as SurveyRow | null
    responseCount = rcRes.count
    pendingCount = pcRes.count
    recentResponses = rrRes.data as ResponseRow[] | null
  } catch (err) {
    return <p className="text-sm text-red-600">Failed to load survey: {String(err)}</p>
  }

  if (!survey) {
    return <p className="text-sm text-red-600">Survey not found.</p>
  }

  const publicUrl = survey.public_url
  const surveyLink = publicUrl ? `${getSiteUrl()}${publicUrl}` : null

  const createdAt = new Intl.DateTimeFormat('en-AU', { month: 'long', year: 'numeric' }).format(new Date(survey.created_at))
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100">{survey.name}</h1>
              <span className={[
      ...
      <div className="rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex items-center justify-between border-b border-zinc-200 px-5 py-4 dark:border-zinc-800">
          ...
          <Link href={`/dashboard/surveys/${id}/responses`} className="text-xs text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100">
            View all →
          </Link>
        </div>
                ...
                return (
                  <tr key={row.id} className="border-t border-zinc-100 hover:bg-zinc-50 dark:border-zinc-800 dark:hover:bg-zinc-800/40">
                    ...
                    <td className="px-5 py-3">
                      <Link href={`/dashboard/surveys/${id}/responses/${row.id}`} className="hover:underline">
                        {classification}
                      </Link>
                    </td>
