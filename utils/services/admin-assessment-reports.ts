...
export async function listAdminAssessmentReports(input: {
  adminClient: AdminClient
  assessmentId: string
...
export async function getAdminAssessmentReport(input: {
  adminClient: AdminClient
  assessmentId: string
  reportId: string
...
export async function createAdminAssessmentReport(input: {
  adminClient: AdminClient
  assessmentId: string
  payload: {
    name?: string
...
export async function updateAdminAssessmentReport(input: {
  adminClient: AdminClient
  assessmentId: string
  reportId: string
  payload: {
...
export async function duplicateAdminAssessmentReport(input: {
  adminClient: AdminClient
  assessmentId: string
  reportId: string
}): Promise<ServiceResult<{ report: AssessmentReportRecord }>> {
...
export async function getAdminAssessmentReportPreview(input: {
  adminClient: AdminClient
  assessmentId: string
  reportId: string
  submissionId: string
...
export async function uploadAdminAssessmentReportAsset(input: {
  ...
  const report = await getAdminAssessmentReport({
    adminClient: input.adminClient,
    assessmentId: input.assessmentId,
    reportId: input.reportId,
  })
  ...
  const assetPath = `assessment-reports/${input.assessmentId}/${input.reportId}/${uuid}.${ext}`

  const { error } = await input.adminClient.storage
    .from('org-assets')
    .upload(assetPath, input.file, { contentType: input.file.type, upsert: false })

  if (error) {
    return { ok: false as const, error: 'upload_failed' }
  }

  const { data: urlData } = input.adminClient.storage.from('org-assets').getPublicUrl(assetPath)
