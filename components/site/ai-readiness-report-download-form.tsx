    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (body?.error === 'report_unavailable') {
          throw new Error('The framework is temporarily unavailable. Please try again shortly.')
        }
  ...
  return (
    <form className="mt-7 space-y-5" onSubmit={handleSubmit} noValidate>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        >
          {isSubmitting ? 'Preparing download...' : unlocked ? 'Details confirmed' : 'Unlock framework'}
        </button>
